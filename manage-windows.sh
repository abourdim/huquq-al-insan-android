#!/usr/bin/env bash
# ============================================================================
#  Huquq al-Insan Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/27-huquq-al-insan}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.huquqalinsan}"
AVD_NAME="${AVD_NAME:-huquq_al_insan_test}"
...
cmd_test() {
  ...
  local out="/tmp/huquq-al-insan-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Huquq al-Insan Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
